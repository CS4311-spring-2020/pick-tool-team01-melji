...
from PyQt5.QtWidgets import QApplication, QWidget, QGridLayout, QLabel, QPushButton, QHBoxLayout, QVBoxLayout, \
    QCheckBox, QAction, QFrame, QMainWindow, QSizePolicy
...
class GetNodeWidgets(QFrame):
    def __init__(self, log_entry, parent=None):
        ...
        self.arrayofsamplewidgets = [VisibilityButton(), LogRandIDTextWidget(), TextWidget(log_entry.identifier),
                                     TextWidget(log_entry.timestamp), TextWidget(log_entry.content),
                                     RandEventTeamWidget(), RandEventTeamWidget(), LogRandNameTextWidget(),
                                     TextWidget(log_entry.host), LogRandNameTextWidget(), IconWidget(),
                                     RandomFileTextWidget(), RandVectorWidget()]
        # self.arrayofsamplewidgets = [TextWidget("Hello")]
...
class TextWidget(QFrame):
    ...
    def __init__(self, text, parent=None):
        ...
        # self.setMinimumHeight(layout.minimumSize())
...
class LogRandIDTextWidget(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        layout.setSpacing(0)
        self.setStyleSheet("border: 1px solid black;")
        self.textlable = QLabel(str(self.value))
        layout.addWidget(self.textlable)
        self.setLayout(layout)
        self.setMaximumHeight(heightoftextrow)
        return
